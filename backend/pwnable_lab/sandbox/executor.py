...
def auto_fmt_got_overwrite_in_container(
    data: bytes, *, settings: _ExecutorSettings
) -> dict:
    """일회용 컨테이너 안에서 포맷스트링 GOT 덮어쓰기 자동 익스(셸 증명까지)를 수행한다.

    fmt 위치 확정·PTY 셸 증명이 실행 프로세스 안에서 일어나야 하므로 CLI
    ``--auto-fmt-got-overwrite`` 로 위임한다(위치는 자체 확정, offset 불필요).
    """
...
def auto_ret2win_pie_in_container(
    ...
    """일회용 컨테이너 안에서 PIE 자동 ret2win(base 관측→rebase→셸 증명)을 수행한다.

    로드 base 관측(``resolve_pie_base``)·ASLR-off 검증이 모두 실행 프로세스 안에서
    일어나야 하므로 CLI ``--auto-ret2win-pie`` 로 위임한다.
    """

    return _run_container(
        settings, ["--stdin", "--auto-ret2win-pie", "--offset", str(offset)], data
    )


def auto_ret2system_pie_in_container(
    data: bytes, *, offset: int, settings: _ExecutorSettings
) -> dict:
    """일회용 컨테이너 안에서 PIE 자동 ret2system(base 관측→rebase→셸 증명)을 수행한다.

    win 함수가 없는 PIE 를 위한 경로. 로드 base 관측·ASLR-off 검증이 실행 프로세스
    안에서 일어나야 하므로 CLI ``--auto-ret2system-pie`` 로 위임한다.
    """

    return _run_container(
        settings, ["--stdin", "--auto-ret2system-pie", "--offset", str(offset)], data
    )
