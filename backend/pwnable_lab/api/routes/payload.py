...
from pwnable_lab.api.schemas import (
    CyclicFindRequest, CyclicFindResponse, CyclicRequest, CyclicResponse,
    OverflowRequest, OverflowResponse, PackRequest, PackResponse,
)
...
from pwnable_lab.payload.pack import (
    RopStep, build_overflow, hexdump_payload, p32, p64,
)
...
def make_cyclic(req: CyclicRequest,
                settings: Settings = Depends(get_config)) -> CyclicResponse:
    if req.length > settings.max_cyclic_length:
        raise AnalysisError(f"length 가 한계({settings.max_cyclic_length})를 초과했습니다.")
    pattern = cyclic(req.length, n=req.n)
    ascii_ = "".join(chr(b) if 0x20 <= b < 0x7F else "." for b in pattern)
    return CyclicResponse(length=req.length, pattern_hex=pattern.hex(), pattern_ascii=ascii_)
...
def find_cyclic(req: CyclicFindRequest) -> CyclicFindResponse:
    ...
    try:
        if raw.lower().startswith("0x"):
            offset = cyclic_find(
                int(raw, 16), n=req.n, max_length=65_536
            )
        else:
            offset = cyclic_find(
                raw.encode(), n=req.n, max_length=65_536
            )
...
def overflow(req: OverflowRequest) -> OverflowResponse:
    payload = build_overflow(
        req.padding, req.target, bits=req.bits,
