export function Icon({ name, size = 18 }) {
  const paths = {
    ...
    wrench: (
      <>
        <path d="M14.5 6.5a4 4 0 0 0-5-5L12 4 9 7 6.5 4.5a4 4 0 0 0 5 5L4 17a2.1 2.1 0 0 0 3 3l7.5-7.5a4 4 0 0 0 5-5L17 10l-3-3 2.5-2.5" />
      </>
    ),
    flag: (
      <>
        <path d="M5 21V4m0 1c5-3 8 3 14 0v9c-6 3-9-3-14 0" />
      </>
    ),
    download: (
      <>
...
export function StatusBadge({ status, compact = false }) {
  const [symbol, label, tone] = statusMeta[status] || ['?', 'Unknown', 'unknown'];
  return (
    <span
      className={`status-badge status-${tone} ${compact ? 'compact' : ''}`}
      aria-label={`Analysis status: ${label}`}
    >
